export default {
	...
	getImageLink: async () => {
		const imageData = await getImageData.run();
	...
	onOrderSelected: async () => {
		...
		storeValue('currentSelectedImageId', tbl_orders.selectedRow.image_id);
		showModal('mdl_orderDetails')
		await this.getOrderProducts();
		await this.getOrderTrack()
		const imageData = await this.getImageLink();
		storeValue('currentImageData', imageData);
	...
	statusColor: (status) => {
		...
		if (status === 'unread' || status === 'PACKED') {
